      ...
      {playbooks.length === 0 ? (
        ...
        <ul className="space-y-1">
          {playbooks.map((pb, i) => (
            <li
              key={pb.id}
              className="flex items-center gap-3 rounded-lg px-3 py-2.5 border border-navy-700 bg-navy-900"
            >
              <span className="w-6 h-6 rounded bg-gold-500 text-navy-950 text-xs font-black flex items-center justify-center shrink-0">
                {i + 1}
              </span>
              <Link to={`/playbooks/${pb.id}`} className="flex-1 text-sm font-semibold text-slate-100 hover:text-gold-400">
                {pb.name}
              </Link>
              ...
              <button
                ...
                className="w-7 h-7 flex items-center justify-center rounded-full text-slate-400 hover:text-gold-400 hover:bg-navy-800"
              >
                ⓘ
              </button>
              <button
                ...
                className="w-7 h-7 flex items-center justify-center rounded-full text-slate-400 hover:text-red-400 hover:bg-navy-800"
              >
                🗑
              </button>
            </li>
