...
export default function OptionRouteToolbar({
  optionRoute,
  activeBranchIndex,
  onSetActiveBranch,
  onUpdate,
  ...
  const branch = optionRoute.branches[activeBranchIndex]
  const setBranchPoint = (index, patch) =>
    onUpdate({
      branches: optionRoute.branches.map((b, i) =>
        i === activeBranchIndex ? { ...b, points: b.points.map((pt, j) => (j === index ? { ...pt, ...patch } : pt)) } : b,
      ),
    })
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-xs font-bold text-gold-400 uppercase truncate">Option Route selected</span>
        <div className="flex items-center gap-1 shrink-0">
          {isCompact && (
            ...
            >
              {expanded ? '▾ Hide options' : '▴ Edit branches'}
            </button>
          )}
      ...
      {(!isCompact || expanded) && (
        <div className="p-3">
          <p className="text-xs font-bold text-gold-400 uppercase mb-1.5">Branches</p>
          <div className="flex flex-wrap gap-1.5 mb-2">
            {optionRoute.branches.map((b, i) => (
              <button
                key={i}
                onClick={() => onSetActiveBranch(i)}
                className={`px-3 py-2 rounded text-xs font-bold ${
                  i === activeBranchIndex ? 'bg-gold-500 text-navy-950' : 'bg-navy-950 border border-navy-700 text-slate-300'
                }`}
              >
                Branch {i + 1}
              </button>
            ))}
            <button onClick={onAddBranch} className="px-3 py-2 rounded text-xs font-bold bg-navy-800 border border-navy-700 text-gold-400">
              + Add Branch
            </button>
          </div>
          {optionRoute.branches.length > 1 && (
            <button
              onClick={() => onDeleteBranch(activeBranchIndex)}
              className="w-full mb-2 py-2 rounded text-xs font-semibold text-red-400 hover:text-red-300 border border-navy-700"
            >
              Delete Branch {activeBranchIndex + 1}
            </button>
          )}

          <p className="text-xs text-slate-400 mb-2">{branch.points.length} point(s) on this branch</p>

          <div className="flex flex-wrap gap-2 mb-2">
            <button
              onClick={() => onUpdate({ branches: optionRoute.branches.map((b, i) => (i === activeBranchIndex ? { ...b, points: b.points.slice(0, -1) } : b)) })}
              disabled={branch.points.length === 0}
              className="px-2.5 py-2.5 rounded text-xs font-semibold text-slate-300 hover:text-white disabled:opacity-40"
            >
              Undo Point
            </button>
            <button
              onClick={() => onUpdate({ branches: optionRoute.branches.map((b, i) => (i === activeBranchIndex ? { ...b, points: [] } : b)) })}
              disabled={branch.points.length === 0}
              className="px-2.5 py-2.5 rounded text-xs font-semibold text-slate-300 hover:text-white disabled:opacity-40"
            >
              Clear Branch
            ...
            <button
              onClick={onDeleteOptionRoute}
              className="px-2.5 py-2.5 rounded text-xs font-semibold text-red-400 hover:text-red-300 ml-auto"
            >
              🗑 Delete Route
            </button>
          </div>
          ...
          {showAdvanced && (
            <div className="mb-2 rounded border border-navy-700 bg-navy-950/60 p-2.5">
              ...
              <div className="grid grid-cols-8 gap-1.5 mb-3">
                {PALETTE.map((c) => (
                  <button
                    ...
                    title={c}
                    className={`aspect-square rounded ${optionRoute.color === c ? 'ring-2 ring-white' : ''}`}
              ...
              <label className="flex items-center gap-2 text-xs font-semibold text-slate-300 py-2 mb-2">
                <input
                  type="checkbox"
                  checked={!!optionRoute.curved}
                  onChange={(e) => onUpdate({ curved: e.target.checked })}
                  className="accent-gold-500 w-5 h-5"
                />
                Smooth this branch (line smoothing)
              </label>

              {branch.points.length === 0 ? (
                <p className="text-xs text-slate-500">Draw this branch first, then style each segment here.</p>
              ) : (
                    ...
                    <li key={i} className="border-b border-navy-800 pb-2 last:border-0">
                      ...
                      <div className="flex gap-1 mb-1">
                        {LINE_STYLES.map((s) => (
                          <button
                            key={s.key}
                            onClick={() => setBranchPoint(i, { style: s.key })}
                      ...
                      <div className="flex gap-1">
                        {SEGMENT_END_CAPS.map((c) => (
                          <button
                            key={c.key}
                            onClick={() => setBranchPoint(i, { endCap: c.key })}
